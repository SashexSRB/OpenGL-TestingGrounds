...
//========================================================================
void pixel (const int x, const int y, const int c) {      // draw a pixel at x/y with rgb
    int rgb[3];
    switch (c) {
        ...
        default: printf("ERR: Color undefined.\n");
    }
...
void clearBackground() {
    for (int y = 0; y < SH; y++) {
        for (int x = 0; x < SW; x++) pixel(x,y,8);
    }
}
...
void display() {
    ...
    T.fr1 = glutGet(GLUT_ELAPSED_TIME);        // 1000ms per second
    glutPostRedisplay();
}
